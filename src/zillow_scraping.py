...
class ZillowScraping:
    ...
    def parse_ticks_data(self, element, type):
        try:
            ...
            points = []
            if type == 'YEARS':
                points = [{'x': float(t['points'][0])+13.36, 'y': t['label']}
                          for t in ticks]
            elif type == 'PRICES':
    ...
    def parse_svg(self, svg):
        try:
            ...
            y_m, y_b = self.get_equation_params(prices)
            x_m, x_b = self.get_equation_params(years)
...
def main():
    url = "https://www.zillow.com/homedetails/24410-Poinsettia-Dr-Lake-Elsinore-CA-92532/303947826_zpid/"
